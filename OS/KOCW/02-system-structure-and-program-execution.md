...
사용자 프로그램이 사용 중이면, Mode Bit이 1인 상태라 I/O에 접근이 불가하다. 운영 체제에 해당하는 메모리 주소로 넘어가야 하는데, 이를 위해 **프로그램이 직접 인터럽트 라인을 세팅하는 Instruction을 실행**함. 이를 Trap이라고 부름. 다른 말로 소프트웨어 인터럽트.
